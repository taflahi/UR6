...
class URAlgorithm(val ap: URAlgorithmParams)
        ...
        val recs = searchHits.getHits.map { hit =>
          if (withRanks) {
            ...
            ItemScore(hit.getId, hit.getScore.toDouble,
              ...
              if (hit.getSource.containsKey("view")) {
                val source_list = hit.getSource.get("view").asInstanceOf[java.util.ArrayList[String]]
                val source_name = Map(source_list.asScala.toList map { s =>
                  {
                    val theMap = splitViewItem(s)
                    (theMap(0), theMap(1))
                  }
                }: _*)
                if (!source_name.isEmpty) {
                  Some(source_name)
                } else Some(Map(("undefined", "undefined"), ("undefined", "undefined")))
              } else Some(Map(("undefined", "undefined"), ("undefined", "undefined"))))
          } else {
  ...
  def splitViewItem(viewItem: String) = {
    viewItem.split(":", 2)
  }
